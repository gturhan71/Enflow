...
> Durum: **Faz A + B + C + D tamam** (2026-08-28) — plan kapsamının tamamı. **Sürüm artırılmadı** (kullanıcı kararı 2026-08-28: Faz C mimari değişiklik olsa da `APP_VERSION` v2.4.0'da kaldı; bump ayrıca konuşulacak).
...
> - Faz E (DMO kanalı): `profitabilityDmo.ts` + `GET /profitability/dmo` (`requireEntitlement('DMO_MODULE')` — lisans yoksa 402). `DmoOrder` snapshot'larını döneme (ay/çeyrek/yıl/kurum) kovalar: ciro/maliyet/brüt kâr/risturn/komisyon/net kâr/net marj% + kârsız sayısı + değerlendirme pipeline'ı. **Proje kümülatifine KARIŞMAZ** (ayrı ekonomi). Modülde üst sekme çubuğu "Projeler | DMO Kanalı" — DMO sekmesi yalnız entitlement aktifse render edilir (`getPluginEntitlements`); `src/modules/profitability/DmoChannelTab.tsx`.
> - Birim testleri 36, `tsc` 0 (fe+be), `audit:roles` 0 ERROR/WARN, backend unit 166/166. Canlı: `/dmo` 200 (tenant-1 DMO_MODULE aktif, 4 sipariş / 10.25% net marj / 2 kârsız); lisans DEVRE DIŞI iken `/dmo` → **402**, `/summary` etkilenmiyor (200).
>
> **RBAC (2026-08-28):** `PROFITABILITY_VIEW` yeni bir izin — yalnız kullanıcı oluşturulurken atandığından, var olan kurulumlara `backend/src/scripts/backfill-profitability-view-permission.ts` (idempotent, `governance/role-matrix.ts` kaynaklı, ADDITIVE) ile işlenir. dev.db'de 4 kullanıcı güncellendi (SALES_MGR/PROJECT_MGR/FINANCE_MGR/BACKUP_ADMIN). Playwright `ui-access` "Kârlılık menüsü": **20/20** (visible: GM/SALES_MGR/FINANCE_MGR/PROJECT_MGR/BACKUP_ADMIN; kalan 15 hidden). (`cross_tenant` setup login'i bu dev.db'de 401 — önceden var olan ortam boşluğu, kârlılıkla ilgisiz.)
