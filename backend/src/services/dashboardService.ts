...
export async function computeDashboard(tenantId: string, userId?: string) {
  ...
  const [opps, tenders, guarantees, invoices, milestones, prs, projects, bottlenecks, myTasks, unread] = await Promise.all([
  ...
  return {
    ...
    timeSensitive: { tenderDeadlines, guaranteeExpiries, guaranteeRequests, costApprovalsPending, invoicesDue, milestonesDue },
    management: {
      bottlenecks,
      tenderPipeline,
