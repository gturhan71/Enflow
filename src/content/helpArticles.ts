...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: 'Şartname ↔ Ürün Uygunluk (yapay zekâ)', body: 'Bu sekmede teknik şartnameyi ve ürün başına bir veya birden çok ürün broşürünü/specsheet\'ini yüklersiniz. Teknik şartname fırsat oluşturulurken sisteme girildiyse otomatik olarak gelir (işaretini kaldırıp elle de yükleyebilirsiniz). Şartnamede birden fazla ürün varsa her ürün ayrı bir grup olur; aynı ürün için rakip markaların specsheet\'lerini yan yana ekleyebilirsiniz. Sistem her şartname maddesinin her aday tarafından karşılanıp karşılanmadığını (Karşılıyor / Kısmen / Karşılamıyor / Belirsiz) kanıt alıntısıyla birlikte bir matris ve indirilebilir xlsx raporu olarak verir, ürün başına "önerilen marka"yı işaretler; öneriyi tek tıkla BoM\'a kalem olarak aktarabilirsiniz. NOT: Bu özellik yalnızca Ayarlar → Entegrasyonlar bölümünden bir yapay zekâ (YZ) API anahtarı tanımlandığında çalışır; anahtar yoksa "Karşılaştır" butonu pasiftir.' },
    ],
  },
  {
    moduleId: 'sales-support',
    summary: 'Kamu ihalelerinde şartname analizi, uygunluk kontrol listesi ve teminat takibi.',
    audience: 'Satış Destek ve İYB (İhale Yönetim Birimi) ekibi.',
    sections: [
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: 'İlk kurulumda önerilen sıra', body: '1) **Lisans Planları**\'nda Tenant ID\'nizi kopyalayıp lisans anahtarınızı aktifleştirin (deneme sürümüyle de devam edebilirsiniz). 2) **Birimler**\'de "Varsayılan Şablonu Yükle" ile 8 standart birimi (Satış, Presales, Finans, İGB, Üst Yönetim, KSU, KY, İYB) ve varsayılan onay/iş akışı şablonunu tek tıkla oluşturun; organizasyonunuza özgü ek birimler (Proje, Satın Alma, Hukuk vb.) varsa "Yeni Birim" ile elle ekleyin. 3) **Kullanıcılar**\'da ekibinizi ekleyip rol + birim atayın. 4) **İş Akışı**\'nda otomatik oluşan şablonu gözden geçirin, gerekirse adım aç/kapa yapın. Tam adım adım anlatım (işletim sistemine göre kurulum dahil): kurulum paketindeki `install/ILK_KURULUM_KILAVUZU.md`.' },
      { heading: 'Nasıl kullanılır', body: 'Yeni bir kullanıcı eklerken önce **Birimler**\'de birimin var olduğundan emin olun, ardından **Kullanıcılar**\'da ekleyip rolünü atayın. ⚠️ **Rol seçmek otomatik izin vermez** — yeni kullanıcı varsayılan olarak yalnız Dashboard\'u görür; **Yetkiler**\'den rolüne uygun modülleri elle açmanız gerekir (modül grupları tek switch ile topluca açılabilir). Yapay zekâ destekli analizleri (şartname/sözleşme) kullanmak için **Entegrasyonlar**\'da kendi sağlayıcı bilginizi (adres/anahtar/model) girmeniz gerekir — girilmezse sistem otomatik bir örnek/mock sonuçla çalışır.' },
      { heading: 'Lisans türleri', body: '**Lisans Planları**\'ndaki anahtar şirketinizin genel abonelik planını belirler (Tenant ID\'ye bağlıdır — şirket adı değil, ekrandaki Tenant ID\'yi lisans sağlayıcınıza iletin). Sanal agent\'lar ve bazı ek modüller (ör. DMO Kataloğu) **ayrı ayrı lisanslanır** — bunlar Ayarlar → Sanal Agentlar ekranından etkinleştirilir, abonelik planından bağımsızdır.' },
    ],
  },
  {
