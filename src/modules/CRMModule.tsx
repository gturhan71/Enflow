...
const CRMModule = ({
  ...
  const [contactForm, setContactForm] = useState<{ id?: string; name: string; role: ContactRole; title: string; email: string; phone: string; isPrimary: boolean }>({ name: '', role: 'OTHER', title: '', email: '', phone: '', isPrimary: false });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {customerSearch.filteredItems.map(customer => (
            ...
            >
              ...
              <button
                onClick={() => { setContactsModal(customer); resetContactForm(); }}
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-medium mt-0.5">{contactsModal.name}</p>
                </div>
                <button onClick={() => setContactsModal(null)} className="p-2 hover:bg-slate-100 rounded-xl transition-all"><X size={18} /></button>
              </div>
              ...
              <form onSubmit={handleSaveContact} className="p-6 border-t border-slate-100 space-y-3 shrink-0">
                ...
                <div className="flex gap-3">
                  {contactForm.id && (
                    <button type="button" onClick={resetContactForm} className="flex-1 py-2.5 text-xs font-black text-slate-500 uppercase tracking-widest hover:bg-slate-100 rounded-xl transition-all">Vazgeç</button>
                  )}
