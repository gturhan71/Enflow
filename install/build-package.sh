...
cp "$HERE/install.sh" "$HERE/install.ps1" "$HERE/wizard.mjs" "$HERE/README.md" "$HERE/.env.example" "$PKG/"
chmod +x "$PKG/install.sh" 2>/dev/null || true

( cd "$STAGE" && zip -rq "$ZIP" enflow-installer )
rm -rf "$STAGE"
echo "✓ Kurulum paketi: $ZIP"
echo "  İçerik: install.sh · install.ps1 · wizard.mjs · README.md · .env.example"
echo "  Kullanım (hedef makinede): açın → ./install.sh  (Linux/macOS)  ·  .\\install.ps1  (Windows)"
